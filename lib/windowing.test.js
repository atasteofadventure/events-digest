...
test('weekday event later this week → thisWeek', () => {
  ...
  assert.equal(bucketFor('2026-06-04T23:30:00', THU), 'thisWeek');   // Thu late
});

test('ALL of Friday through Sun → thisWeekend (Friday is a weekend day)', () => {
  assert.equal(bucketFor('2026-06-05T00:00:00', THU), 'thisWeekend'); // Fri all-day
  assert.equal(bucketFor('2026-06-05T12:00:00', THU), 'thisWeekend'); // Fri noon
...
test('following Mon–Thu → nextWeek', () => {
  assert.equal(bucketFor('2026-06-08T18:00:00', THU), 'nextWeek');    // Mon
  assert.equal(bucketFor('2026-06-11T12:00:00', THU), 'nextWeek');    // Thu noon
});

test('following Fri–Sun → nextWeekend', () => {
  assert.equal(bucketFor('2026-06-12T12:00:00', THU), 'nextWeekend'); // Fri noon
  assert.equal(bucketFor('2026-06-13T14:00:00', THU), 'nextWeekend'); // Sat
});

test('past/undated → past (dropped); far-future → later (kept)', () => {
  assert.equal(bucketFor('2026-06-01T10:00:00', THU), 'past');   // last Mon, before run day
  assert.equal(bucketFor('not-a-date', THU), 'past');            // unparseable date
  assert.equal(bucketFor('2026-07-01T10:00:00', THU), 'later');  // weeks out
  assert.equal(bucketFor('2026-12-31T10:00:00', THU), 'later');  // months out
});

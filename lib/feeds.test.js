...
test('nycParksRssToEvents infers Free from a "Free …" category and sets category', () => {
  ...
  assert.equal(concert.category, 'music_nightlife');
...
test('parsers apply default_category to every event', () => {
  const [e] = icsToEvents(ICS, { sourceName: 'Makeville Studio', defaultCategory: 'workshops_classes' });
  assert.equal(e.category, 'workshops_classes');

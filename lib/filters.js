...
// A listing the organizer has already withdrawn.
const CANCELLED = /\b(cancel+ed|postponed|sold out|rescheduled)\b/i;

function dropReason(e) {
  if (!e) return null;
  if (CANCELLED.test(String(e.name || ''))) return 'cancelled';
  const t = text(e);

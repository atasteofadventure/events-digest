...
// The build guarantees every event has a real URL (lib/curate.js drops the
// rest). Never fall back to a web search — that produced junk links.
function hrefFor(e) {
  const u = (e && e.url ? String(e.url) : '').trim();
  return /^https?:\/\//i.test(u) ? u : DASHBOARD_URL;
}

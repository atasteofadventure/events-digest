...
const ev = (o) => Object.assign({ name: 'E', venue: 'V', via: ['x'], url: 'https://example.org/e' }, o);
...
test('drops events without a real URL (never a web-search link)', () => {
  const out = curate([
    ev({ name: 'No Link', dateISO: '2026-06-05T20:00:00', url: '' }),
    ev({ name: 'Bad Link', dateISO: '2026-06-05T20:00:00', url: 'see website' }),
    ev({ name: 'Good', dateISO: '2026-06-05T20:00:00', url: 'https://ok' }),
  ], RUN);
  assert.deepEqual(out.thisWeekend.map(e => e.name), ['Good']);
  assert.equal(out._stats.noUrl, 2);
});

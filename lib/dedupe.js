...
// Ambiguous band for the LLM tie-breaker: same day, venue not contradicting,
// names related but below the auto-merge thresholds (or blocked only by a
// discriminator word). These are the pairs a human would have to read to
// decide; everything clearer is settled by compare() alone.
const REVIEW_MIN = 0.45;
function candidatePairs(events, opts) {
  ...
  for (const list of byDay.values()) {
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        const r = compare(a, b);
        if (r.dup) continue;
        if (r.reason !== 'dissimilar' && !r.reason.startsWith('discriminator:')) continue;
        // Discriminator-blocked pairs carry no score; compute one for ranking.
        let score = r.score;
        if (score === undefined) {
          const na = normName(a.name, a.venue), nb = normName(b.name, b.venue);
          const ta = nameTokens(na), tb = nameTokens(nb);
          score = Math.max(dice(na, nb), jaccard(ta, tb), containment(ta, tb));
        }
        if (score < REVIEW_MIN) continue;
        out.push({ a, b, score, reason: r.reason });
      }
    }
  }
...
// Pair key for verdict lookup: order-independent on event ids.
...
module.exports = { compare, isDuplicate, candidatePairs, pairKey, REVIEW_MIN, exactKey, normName, normVenue, canonicalUrl, venuesCompatible, dice, jaccard, containment, THRESHOLDS: T };

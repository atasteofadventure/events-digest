...
const {
  ...
  eventbriteApiToEvents, isNycAddress,
} = require('./feeds');
...
function apiEvent(overrides) {
  return Object.assign({
    id: '1997614339263',
    name: { text: 'Lectures on Tap - "AI vs. the MD"' },
    summary: 'Professors, experts, and storytellers giving thought-provoking lectures in NYC bars.',
    url: 'https://www.eventbrite.com/e/lectures-on-tap-ai-vs-the-md-tickets-1997614339263',
    start: { timezone: 'America/New_York', local: '2026-09-14T18:30:00' },
    status: 'live', online_event: false, is_free: false,
    venue: { name: 'West Village', address: { city: 'New York', region: 'NY', postal_code: '10014' } },
    ticket_availability: {
      has_available_tickets: true, is_sold_out: false, start_sales_date: null,
      minimum_ticket_price: { major_value: '41.32', value: 4132 },
      maximum_ticket_price: { major_value: '41.32', value: 4132 },
    },
  }, overrides);
}
const NOW_API = '2026-09-10T12:00:00';

test('isNycAddress: five boroughs by city or ZIP, not other cities or upstate NY', () => {
  assert.equal(isNycAddress({ city: 'New York', region: 'NY', postal_code: '10014' }), true);
  assert.equal(isNycAddress({ city: 'Brooklyn', region: 'NY', postal_code: '11249' }), true);
  assert.equal(isNycAddress({ city: 'Astoria', region: 'NY', postal_code: '11102' }), true); // neighborhood as city
  assert.equal(isNycAddress({ city: 'Staten Island', region: 'NY' }), true);
  assert.equal(isNycAddress({ city: 'Buffalo', region: 'NY', postal_code: '14201' }), false);
  assert.equal(isNycAddress({ city: 'Brooklyn', region: 'MI', postal_code: '49230' }), false);
  assert.equal(isNycAddress({ city: 'Chicago', region: 'IL', postal_code: '60614' }), false);
  assert.equal(isNycAddress(undefined), false);
});

test('eventbriteApiToEvents maps the v3 event shape to the digest event shape', () => {
  const [e] = eventbriteApiToEvents([apiEvent({})],
    { sourceName: 'Lectures on Tap', defaultCategory: 'talks_lectures', nowISO: NOW_API });
  assert.equal(e.name, 'Lectures on Tap - "AI vs. the MD"');
  assert.equal(e.dateISO, '2026-09-14T18:30:00');
  assert.equal(e.time, '6:30 PM');
  assert.equal(e.venue, 'West Village, Manhattan');
  assert.equal(e.price, '$41.32');
  assert.equal(e.url, 'https://www.eventbrite.com/e/lectures-on-tap-ai-vs-the-md-tickets-1997614339263');
  assert.equal(e.why, 'Professors, experts, and storytellers giving thought-provoking lectures in NYC bars.');
  assert.equal(e.category, 'talks_lectures');
  assert.equal(e.source, 'Lectures on Tap');
});

test('eventbriteApiToEvents keeps only NYC, live, in-person events that still have tickets', () => {
  const soldOutFlag = { has_available_tickets: false, is_sold_out: true };
  // Observed 2026-09-10: Eventbrite leaves is_sold_out false on events whose
  // page says "Sold out"; has_available_tickets:false is the reliable signal.
  const soldOutNoFlag = { has_available_tickets: false, is_sold_out: false, start_sales_date: null };
  // The v3 API returns start_sales_date as an object, not a string.
  const notYetOnSale = { has_available_tickets: false, is_sold_out: false,
    start_sales_date: { timezone: 'America/New_York', local: '2026-09-20T10:00:00', utc: '2026-09-20T14:00:00Z' } };
  const salesOpened = { has_available_tickets: false, is_sold_out: false,
    start_sales_date: { timezone: 'America/New_York', local: '2026-08-05T01:30:00', utc: '2026-08-05T05:30:00Z' } };
  const evts = eventbriteApiToEvents([
    apiEvent({ id: 'keep-manhattan' }),
    apiEvent({ id: 'keep-bk', url: 'https://www.eventbrite.com/e/bk-2',
      venue: { name: 'Williamsburg', address: { city: 'Brooklyn', region: 'NY', postal_code: '11249' } } }),
    apiEvent({ id: 'keep-presale', url: 'https://www.eventbrite.com/e/presale-3', ticket_availability: notYetOnSale }),
    apiEvent({ id: 'chicago', venue: { name: 'Old Town', address: { city: 'Chicago', region: 'IL', postal_code: '60610' } } }),
    apiEvent({ id: 'online', online_event: true }),
    apiEvent({ id: 'canceled', status: 'canceled' }),
    apiEvent({ id: 'sold-flag', ticket_availability: soldOutFlag }),
    apiEvent({ id: 'sold-noflag', ticket_availability: soldOutNoFlag }),
    apiEvent({ id: 'sold-after-sales-opened', ticket_availability: salesOpened }),
    apiEvent({ id: 'tappan', venue: { name: "The Old '76 House", address: { city: 'Tappan', region: 'NY', postal_code: '10983' } } }),
    apiEvent({ id: 'no-venue', venue: null }),
  ], { sourceName: 'X', nowISO: NOW_API });
  assert.deepEqual(evts.map((e) => e.url), [
    'https://www.eventbrite.com/e/lectures-on-tap-ai-vs-the-md-tickets-1997614339263',
    'https://www.eventbrite.com/e/bk-2',
    'https://www.eventbrite.com/e/presale-3',
  ]);
  assert.equal(evts[1].venue, 'Williamsburg, Brooklyn');
});

test('eventbriteApiToEvents does not repeat a borough the venue name already carries', () => {
  const [e] = eventbriteApiToEvents([apiEvent({ venue: { name: 'Midtown Manhattan',
    address: { city: 'New York', region: 'NY', postal_code: '10019' } } })], { sourceName: 'X', nowISO: NOW_API });
  assert.equal(e.venue, 'Midtown Manhattan');
});

test('eventbriteApiToEvents: free events price as Free; duplicate ids collapse', () => {
  const free = apiEvent({ is_free: true, ticket_availability: { has_available_tickets: true } });
  const evts = eventbriteApiToEvents([free, free], { sourceName: 'X', nowISO: NOW_API });
  assert.equal(evts.length, 1);
  assert.equal(evts[0].price, 'Free');
});

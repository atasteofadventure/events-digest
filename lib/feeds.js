...
// ---------- Eventbrite v3 API (/api/v3/organizers/<id>/events/) ----------
// Answers unauthenticated on www.eventbrite.com and, unlike the organizer page
// (first 12 only), pages through every live event. Used for multi-city
// organizers (Lectures on Tap), so venues are filtered to the five boroughs.

const NYC_CITIES = new Set(['new york', 'new york city', 'nyc', 'manhattan', 'brooklyn',
  'queens', 'bronx', 'the bronx', 'staten island', 'long island city', 'astoria']);
// 110xx is split between Queens and Nassau County, so it is left out.
const NYC_ZIP = /^1(0[0-4]|1[1-46])\d\d$/;
const BOROUGH_BY_CITY = { 'new york': 'Manhattan', 'new york city': 'Manhattan', nyc: 'Manhattan' };

function isNycAddress(addr) {
  if (!addr || !/^(ny|new york)$/i.test(String(addr.region || '').trim())) return false;
  const city = String(addr.city || '').trim().toLowerCase();
  return NYC_CITIES.has(city) || NYC_ZIP.test(String(addr.postal_code || '').trim());
}

// Eventbrite leaves is_sold_out false on some events whose page reads "Sold
// out" (verified against every NYC Lectures on Tap page, 2026-09-10);
// has_available_tickets:false matched sold_out/sales_ended every time. Keep
// only the not-yet-on-sale case.
function ebUnavailable(avail, nowISO) {
  if (!avail) return false;
  if (avail.is_sold_out) return true;
  if (avail.has_available_tickets !== false) return false;
  const s = avail.start_sales_date; // v3 API: { timezone, local, utc }
  const opens = Date.parse((s && typeof s === 'object' ? s.utc : s) || '');
  return !(opens > Date.parse(nowISO));
}

function eventbriteApiToEvents(apiEvents, { sourceName, defaultCategory, nowISO }) {
  const seen = new Set();
  const events = [];
  for (const ev of apiEvents || []) {
    if (!ev || !ev.name || !ev.start || !ev.start.local || seen.has(ev.id)) continue;
    seen.add(ev.id);
    if (!['live', 'started'].includes(ev.status) || ev.online_event) continue;
    const venue = ev.venue || {};
    const addr = venue.address || {};
    if (!isNycAddress(addr)) continue;
    if (ebUnavailable(ev.ticket_availability, nowISO || new Date().toISOString())) continue;
    const city = String(addr.city || '').trim();
    const borough = BOROUGH_BY_CITY[city.toLowerCase()] || city;
    const dateISO = ev.start.local.slice(0, 19);
    events.push(makeEvent({
      sourceName,
      name: ev.name.text || ev.name.html,
      dateISO,
      time: timeLabel(dateISO),
      // Lectures on Tap lists a neighborhood here; the bar is revealed on booking.
      venue: !venue.name ? borough
        : venue.name.toLowerCase().includes(borough.toLowerCase()) ? venue.name
          : `${venue.name}, ${borough}`,
      price: ebPrice({ ...(ev.ticket_availability || {}), is_free: ev.is_free }),
      url: ev.url,
      why: ev.summary,
      category: defaultCategory,
    }));

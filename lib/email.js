...
//
// Scope: only this week + this weekend (today through Sunday; Friday counts as weekend). Events are grouped
...
const CATEGORY_ORDER = ['tech_ai', 'music_nightlife', 'film_screenings', 'art_exhibitions', 'talks_lectures', 'workshops_classes', 'tours_experiences', 'sports_fitness', 'festivals_parties', 'miscellaneous'];
...
// Real event page when we have one; otherwise (build flagged `urlMissing`) an
// explicit web search, labelled as such in the row so it never masquerades as
// an event link.
function hrefFor(e) {
...
function runsThroughLabel(e) {
  if (!e || !e.runsThrough) return '';
  const d = new Date(e.runsThrough + 'T12:00:00');
  return isNaN(d) ? '' : 'runs through ' + MONS[d.getMonth()] + ' ' + d.getDate();
}
...
function eventRow(e) {
  ...
  const meta = [e.time, e.venue, e.neighborhood, e.price, runsThroughLabel(e)]
    .filter(Boolean).map(esc).join(' &middot; ');
  const why = e.why
    ? '<div style="font-size:13px;color:#57534e;font-style:italic;margin-top:2px;">' + esc(e.why) + '</div>'
    : '';
  const nolink = (!/^https?:\/\//i.test(String(e.url || '').trim()))
    ? '<div style="font-size:12px;color:#b45309;margin-top:2px;">No event page in the source; title links to a web search.</div>'
    : '';
  return '' +

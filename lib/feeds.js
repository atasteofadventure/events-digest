...
function jsonLdToEvents(html, { sourceName }) {
  ...
  return found.map((n) => {
    // Drop any timezone offset — event times are already NYC-local on NYC sites.
    const raw = String(n.startDate).replace(/([+-]\d{2}:\d{2}|Z)$/, '');
...
function nycParksRssToEvents(xml, { sourceName, boroughs }) {

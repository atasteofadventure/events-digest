...
test('buildEmailHtml labels a blank-url event and links it to a web search', () => {
  const html = buildEmailHtml(data);   // Noir Film has no url
  assert.match(html, /google\.com\/search/);
  assert.match(html, /No event page in the source/);
});

test('buildEmailHtml shows a "runs through" marker for collapsed series', () => {
  const d = { meta: { title: 'X' }, thisWeek: [
    { name: 'Open Hours', url: 'https://x', dateISO: '2026-06-27T11:00:00', category: 'art_exhibitions', runsThrough: '2026-07-15', occurrences: 12 },
  ] };
  assert.match(buildEmailHtml(d), /runs through Jul 15/);
});

...
// Weekend = ALL of Friday through Sunday (user directive 2026-08-27: Friday belongs
// to the weekend; the old 5 PM Friday split scattered Friday events across two tabs).
function bucketFor(eventDateISO, runDateISO) {
  ...
  const friEve = startOfDay(addDays(mon, 4));                    // Friday 00:00
  const sunEnd = (() => { const s = addDays(mon, 6); s.setHours(23, 59, 59, 999); return s; })();
  const monNext = addDays(mon, 7);
  const friEveNext = startOfDay(addDays(monNext, 4));
